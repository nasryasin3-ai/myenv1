...
class ERPRagSystem:
    ...
    def query(self, question: str, **kwargs) -> dict:
        ...
        # --- NEW AGENTIC DATA ANALYST ROUTING ---
        company = kwargs.get('company')
        if company:
            try:
                from products.models import DataFile
                from django.db.models import Q
                from rag.pandas_agent import PandasAgent
                
                # Fetch recent CSV/Excel files for this company
                data_files = DataFile.objects.filter(
                    Q(uploaded_by__profile__company__name=company) | Q(uploaded_by__profile__company_name=company),
                    file_type__in=['csv', 'excel']
                ).order_by('-uploaded_at')[:3] # limit to 3 latest files to avoid LLM context limit
                
                if data_files.exists():
                    agent = PandasAgent()
                    pandas_answer = agent.analyze(question, list(data_files))
                    if pandas_answer:
                        now_str = datetime.now().isoformat() if isinstance(datetime.now(), datetime) else time.strftime('%Y-%m-%dT%H:%M:%S')
                        return {
                            "question": question,
                            "answer": pandas_answer,
                            "sources": [{"type": "Agentic Data Analyst", "source_text": "تم تحليل البيانات رقمياً باستخدام (Pandas Agent) من الملفات المرفوعة لاستخراج دقيق 100%."}],
                            "timestamp": now_str,
                        }
            except Exception as e:
                print(f"Pandas Agent Routing Error: {e}")
        # --- END AGENTIC ROUTING ---
