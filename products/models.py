...
class CustomDepartment(models.Model):
    ...
    custom_schema = models.JSONField(default=list, blank=True) # [{"name":"price", "type":"number", "label":"Price"}]
...
class Profile(models.Model):
    ...
    PLAN_CHOICES = [
        ...
        ('1_month', '1 Month (8,000 EGP)'),
        ('3_months', '3 Months (23,000 EGP)'),
        ('1_year', '1 Year (90,000 EGP)')
    ]
    SECTOR_CHOICES = [
        ('corporate', 'Corporate / Commercial'),
        ('education', 'Education / Schools / Universities'),
        ('medical', 'Medical / Hospitals / Clinics'),
        ('construction', 'Construction / Real Estate (Villas)'),
    ]
...
class DataFile(models.Model):
    ...
    def __str__(self):
        return f"{self.file.name} (from {self.source_department})"
...
class FinancialAlert(models.Model):
    STATUS_CHOICES = [('danger', 'Danger'), ('warning', 'Warning'), ('safe', 'Safe')]
...
class MarketingCampaign(models.Model):
    STATUS_CHOICES = [('planned','Planned'), ('active','Active'), ('paused','Paused'), ('completed','Completed')]
...
class PurchaseOrder(models.Model):
    STATUS_CHOICES = [('pending','Pending'), ('approved','Approved'), ('received','Received'), ('cancelled','Cancelled')]
...
class LegalContract(models.Model):
    CONTRACT_TYPES = [
        ('service','Service Contract'), ('employment','Employment Contract'),
        ('nda','Non-Disclosure Agreement'), ('partnership','Partnership Agreement'),
        ('supplier','Supply Contract'), ('lease','Lease Agreement'),
    ]
...
class StudentEnrollment(models.Model):
    ...
    status = models.CharField(max_length=50, choices=[('active','Active'), ('graduated','Graduated'), ('transferred','Transferred')], default='active')
    gpa = models.DecimalField(max_digits=4, decimal_places=2, default=0.0)
    grades = models.TextField(blank=True, null=True, help_text="Grade records")
    notes = models.TextField(blank=True)
...
class MedicalInventory(models.Model):
    ...
    category = models.CharField(max_length=100, choices=[('medicine','Medicines'), ('equipment','Medical Equipment'), ('supplies','Supplies')])
...
class EquipmentLog(models.Model):
    ...
    status = models.CharField(max_length=50, choices=[('working','Working'), ('maintenance','Under Maintenance'), ('broken','Broken')], default='working')
...
class ActivityLog(models.Model):
    ...
    description = models.TextField() # Description of what happened
