...
def _create_activity_log(sender, instance, action, **kwargs):
    ...
    desc = f"{action} record ({model_name}) with ID {instance.pk}"
    
    # Optional context mapping
    if model_name == 'LeaveRequest':
        desc = f"{action} leave request for employee {instance.employee.user.username if user else 'Unknown'} — type: {instance.get_leave_type_display()}"
    elif model_name == 'PurchaseOrder':
        desc = f"{action} purchase order for vendor {instance.vendor_name} — amount: {instance.total_amount}"

    ActivityLog.objects.create(
        company=instance.company,
        user=user,
        action_type='create' if action == 'Created' else 'update' if action == 'Updated' else 'delete',
...
def log_deletions(sender, instance, **kwargs):
    _create_activity_log(sender, instance, 'Deleted', **kwargs)
